...
export default function SocialLoginButtons({ mode }: SocialLoginButtonsProps) {
  ...
  const handleSocialLogin = async (provider: 'google') => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`${provider} login not available`);
      const data = await res.json();
      if (data.url) {
        window.location.href = data.url;
      }
    } catch {
      toast.error(`${provider.charAt(0).toUpperCase() + provider.slice(1)} sign-in is not configured yet. Please use email & password.`);
    } finally {
      setLoading(null);
    }
  };
